...
if not defined MIN set min=1 & start "" /min "%~f0" %* & exit
if not "%1"=="" goto %1
...
call "%~f0" :check_steam | find "steamwebhelper.exe" > nul || goto :steam_kill_skip
echo Closing Steam...
start "" %STEAM_EXE% "-shutdown"
:killed_check
timeout /t 1 /nobreak > nul
call "%~f0" :check_steam | find "steamwebhelper.exe" > nul && goto :killed_check
:steam_kill_skip
...
call "%~f0" :check_steam | find %BPM_CHECK% > nul || goto :open_check
...
call "%~f0" :check_steam | find %BPM_CHECK% > nul && goto :closed_check
